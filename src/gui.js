#!/usr/bin/env node
// QVAC Late Night Snack Idea Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29370;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/latenightsnack") {
      try {
        const { ingredients } = await readBody(req);
        if (!ingredients || !ingredients.trim()) {
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ error: "Please fill in the field first" }));
          return;
        }
        const result = await generate(modelId, ingredients.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Late Night Snack Idea Generator GUI ready at http://localhost:${PORT}`);
  });
